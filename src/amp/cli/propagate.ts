...
/** Build real filesystem harness writers at the CLI boundary. */
export function createPropagationHarnessWriters(projectRoot: string): HarnessWriterRegistry {
  const roots = derivePropagationHarnessRoots(projectRoot);
  const cursor = new CursorAdapter({ projectRoot: roots.projectRoot });
  const claudeCode = new ClaudeCodeAdapter({
    basePath: roots.claudeCodeBasePath ?? join(roots.projectRoot, ".claude", "skills"),
  });
  const hermes = new HermesAdapter({ projectRoot: roots.projectRoot });
...
export async function runAmpPropagate(
  ...
  const discoveredConfig = discoverAmpConfig({
    projectRoot,
    env: {
      ...env,
      [AMP_USER_CONFIG_PATH_ENV]:
  ...
  const proceduresDir = join(dirname(discoveredConfig.sources.projectConfigPath ?? configPath), "procedures");
  const registry =
