...
- **wiki/** — (optional, scaffolded via \`ai-memory init --wiki\`) Karpathy LLM Wiki: stable domain knowledge, LLM-owned, interlinked. Ingest via the \`llm-wiki\` skill. Distinct from \`memory/\` (volatile project decisions).
- **sources/** — (optional) immutable raw sources. You read, never write. Binaries in \`sources/assets/\` require a companion \`.md\` source-card to be searchable.
...
Immutable paths (do not write): IDENTITY.md, toolbox/, acp/, rules/, sources/.
...
const SKILL_STUBS: Array<[string, string]> = [
  ...
  ["llm-wiki", "Ingest/query/lint/promote the Karpathy-style LLM Wiki in .ai/wiki/. Requires `ai-memory init --wiki`."],
];
