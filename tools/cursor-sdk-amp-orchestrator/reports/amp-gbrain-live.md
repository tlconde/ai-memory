...
- **Soft-delete cleanup only:** test cleanup uses `delete_page`; pages remain recoverable for ~72h. Residual slugs under `amp/frames/h.*` possible if cleanup fails or operator skips live test finally block. Failure output includes slug, frame id, and operator grep guidance (`formatResidualPageWarning` in `src/amp/gbrain/live-policy.ts`).
